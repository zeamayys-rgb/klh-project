...
| A2-07 | Minor | Daftar Akun | `daftar.html:46, 118` | Dua `<h1>` dalam satu dokumen (form + panel verifikasi email). Hanya satu yang terlihat pada satu waktu (panel lain `hidden`), sehingga lolos "satu h1 terlihat", tetapi outline dokumen memuat dua h1. | Jadikan judul panel verifikasi `<h2>` bergaya h1, atau pindahkan fokus ke judul saat panel berganti. | **Diperbaiki** (10 Jul 2026) |
| A2-08 | Minor | Ajukan Permohonan | `permohonan.html:199, 203` | `aria-label="Hapus <nama berkas>"` tidak meng-escape tanda kutip ganda; nama berkas mengandung `"` dapat memecah atribut (nama sudah di-escape untuk `<` saja). Kasus tepi — unggahan adalah simulasi. | Escape `"` → `&quot;` di `addFiles()`/`renderFiles()`. | **Diperbaiki** (10 Jul 2026) |
| A2-09 | Saran | Konfirmasi | `konfirmasi.html:88-95` | `navigator.clipboard.writeText()` dibungkus try/catch sinkron; penolakan Promise (izin clipboard) tidak tertangani — tombol tetap menampilkan "Tersalin" meski gagal. | Gunakan `.then/.catch` untuk status salin. | **Diperbaiki** (10 Jul 2026) |
| A2-10 | Saran | Keberatan | `keberatan.html:129` | Regex nomor registrasi menerima 4–6 digit (`\d{4,6}`) padahal format resmi 6 digit (`PPID-2026-XXXXXX`); pesan error pun menyebut format XXXXXX. Konsistensi validasi antar-halaman (lacak kini 6 digit). | Perketat ke `\d{6}`. | **Diperbaiki** (10 Jul 2026) |
...
**Tindak lanjut 10 Juli 2026:** empat temuan Minor/Saran tersisa (A2-07 h1 ganda → `<h2>` bergaya h1 · A2-08 escape `"` pada nama berkas di `addFiles()` · A2-09 status salin via `.then/.catch` Promise clipboard · A2-10 regex nomor registrasi diperketat ke `\d{6}`) **diperbaiki dan diverifikasi ulang** — 15 halaman × 2 viewport: nol error JS, nol overflow; uji terarah lolos (satu h1 di `daftar.html`, nomor 4 digit ditolak / 6 digit diterima, aria-label nama berkas ber-kutip tetap utuh, kegagalan clipboard dilaporkan jujur). `klh-ppid-web.zip` diperbarui.
