...
- Verifikasi: Playwright **10 halaman × 2 viewport nol error JS & nol overflow** (404 sidecar `.image-slots.state.json` = bawaan image-slot, benign); dialog `data-ext` teruji SIMPEL/SIDIA/SITARU; tombol Adipura terverifikasi hilang. Belum ditindaklanjuti (butuh konfirmasi klien, kolom Remark BGS): konten menu baru 2.4/4.1/4.4/4.6/5.5/5.6/5.7, target redirect Laboratorium/JDIH/E-Learning, halaman news SP4N-LAPOR & Agenda.
