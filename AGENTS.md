# AGENTS.md — Proyek UI/UX KLH/BPLH (PT Bening Guru Semesta)
...
| Modul | Folder | Status |
|---|---|---|
| 01 · Website Utama (24 hlm) | `klh-website-utama/` | ✅ SELESAI — sumber fondasi & referensi. **Boleh diubah bila perubahan datang dari export Codex Design** (lihat "Integrasi zip dari Codex Design"); di luar itu, jangan diubah tanpa diminta eksplisit |
...
## Integrasi zip dari Codex Design

Bila saya minta menarik/integrasi perubahan dari export Codex Design (zip terbaru di `~/Downloads/`, biasanya bernama `KLH Project Repository*.zip`):

1. **Ambil zip terbaru** dari `~/Downloads/` (urut berdasarkan waktu), ekstrak ke scratchpad — jangan unzip menimpa repo langsung.
2. **Integrasikan penuh ke folder proyek ini** via `rsync` dari hasil ekstrak, termasuk **Modul 01** (`klh-website-utama/`) — perubahan dari Codex Design boleh menimpa modul frozen.
...
- Jangan mengubah isi `klh-website-utama/` (Modul 01 sudah final) — kecuali diminta eksplisit **atau** perubahan berasal dari export Codex Design (lihat "Integrasi zip dari Codex Design").
